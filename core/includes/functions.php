...
function islog($redirect){
	global $user;
	if( !$user->is_logged_in() ){ header('Location: login?action=needlogin&route='.$redirect.''); } else { echo ''; }
}
...
function jeempty($redirect, $premenna){
	if (empty($premenna)){
		header("Location: ".$premenna."");
	} else {
		echo '';
	}
}
